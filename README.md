...
- **七个内置模板**（按意图选择，降低跳出率）：
  - **模板 1（默认）**：标题 → 正文 → 两排产品（首屏先答题，适合品类明确的长尾）
  - **模板 2**：标题 → 第一排产品 → 正文 → 第二排 + FAQ（仅当商品与标题强相关时使用；腕表/戒指/耳机批量生成会自动改用 6/7）
  - **模板 3**：同模板 2 + 扩展正文（长文）
  - **模板 4**：Top Picks + 对比表 + 内链 + 产品（榜单 / Top N）
  - **模板 5**：Hero + 信任条 + Top Picks + 对比 + 产品（高转化榜单）
  - **模板 6**：Hero + 产品 + 正文 + 参考文献 + FAQ（E-E-A-T 信息类）
  - **模板 7**：博客式长文 + References + 文末产品（luxury-life-guides 首选；腕表/戒指/耳机指南推荐）
- **标题与商品对齐**：生成流水线使用最终页面标题（含 AI 生成标题）过滤商品，并按主类目（手机/腕表/戒指/耳机）做品类闸门，避免「标题写表、网格卖手机」。
