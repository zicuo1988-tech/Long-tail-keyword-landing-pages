...
export async function fetchRelatedProducts(
  ...
  const sellableNoPaymentLink = sellable.filter((p) => !isPaymentLinkProductTitle(p.title));
  const pool = matched.length > 0 ? matched : sellableNoPaymentLink;
  if (matched.length === 0 && sellableNoPaymentLink.length > 0) {
    console.warn(
      `[Shopify] 关键词与分类未匹配到商品，使用全店可售列表兜底（已排除 Payment Link 标题，${sellableNoPaymentLink.length} 件，关键词: "${keyword}"）`
    );
  }
