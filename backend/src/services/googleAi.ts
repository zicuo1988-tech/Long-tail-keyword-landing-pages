...
async function generateWithKey(
  ...
  // 检测产品类型（同时考虑 keyword 与 pageTitle，避免腕表/手表页出现手机内容）
  const combinedForProductType = `${keyword} ${pageTitle || ""}`.toLowerCase();
  const isPhoneKeyword = combinedForProductType.includes("phone") || combinedForProductType.includes("smartphone") ||
                         combinedForProductType.includes("mobile") || combinedForProductType.includes("cell");
  const isWatchKeywordCombined = /\b(watch|watches|timepiece|timepieces|wristwatch|horology|chronograph|腕表|手表)\b/.test(combinedForProductType);
  const isRingKeywordCombined = /\b(ring|rings|jewellery|jewelry|首饰)\b/.test(combinedForProductType);
  const isEarbudKeywordCombined = /\b(earbud|earbuds|earphone|earphones|audio|耳机)\b/.test(combinedForProductType);
...
- If the title or keyword is about watches/timepieces (e.g. "watch", "timepiece", "腕表", "手表", "wristwatch", "horology"), the content MUST be about watches ONLY—DO NOT mention phones (Agent Q, Quantum Flip, Metavertu, etc.), Ruby Key, Ruby Talk, or Concierge Service. Writing phone content on a watch page is a SEVERE SEO and UX error.
- If the title/keyword is about wearables, hearables, or earbuds, DO NOT mention phones or phone-only services. If about rings/jewellery, DO NOT mention phones, watches, or earbuds.
...
\n`;
  ...
  const imageOrderNote =
    " URLs are topic-ranked and rotated per page—use a DIFFERENT URL for each <figure>; never repeat the same src twice in one article. Spread images across H2 sections.";
...
- Use ONLY one of these Sanity CDN URLs (copy exactly, including query string). Pick any URL that fits the section; avoid always defaulting to the first in the list.${imageOrderNote}
  ${urls}
...
- You MUST use ONLY the following image URLs (Sanity-hosted content imagery; copy each URL exactly as shown, character-for-character, including ? and &). Do not invent, guess, or substitute other URLs.${imageOrderNote}
  ${urls}
- Each figure MUST use a distinct URL from the list above (no duplicate src attributes in the same article).
- Each image must match the article topic "${keyword}" and product category discussed (no mismatched product types).
- Include at least ONE HTML table in the article (e.g. key features comparison, specifications at a glance, or "What to look for" summary). Use semantic markup:
  <table><thead><tr><th>Feature</th><th>Details</th></tr></thead><tbody><tr><td>...</td><td>...</td></tr></tbody></table>
- Make the article rich and substantial: use multiple H2 sections, several paragraphs, at least one table, and ${figureCount} figures so the page has clear visual and structural variety.
\n`;
})()}
...
- For ALL VERTU product details (specs, prices, features, materials, product names, services): use ONLY the filtered knowledge base above. DO NOT invent or use external product facts.
- DO NOT use information from products that are NOT listed in the "RELEVANT PRODUCTS" section above
- For general topic context (e.g. industry background, why the category matters, how to choose): you MAY add accurate, general information to enrich the article, as long as it is factual and does not invent brand-specific or product-specific claims. Product facts remain KB-only.
...
- For VERTU product information: ONLY use facts, specifications, prices, materials, and features explicitly stated in the knowledge base above. All product names, prices, specs, and features MUST match the knowledge base exactly.
- If a product detail is missing from the knowledge base, omit it rather than inventing it. DO NOT use common industry knowledge to fill product specs (e.g. if KB says "5ATM", do not write "10ATM").
- For general topic context only (e.g. why the category matters, how to choose, industry context): you MAY add accurate, general information to enrich the article, as long as you do not invent product or brand-specific claims.
- DO NOT make assumptions or inferences about VERTU products; do not use external knowledge for product details
...
- Keep content general and relevant to the keyword, focusing on VERTU's brand values and luxury technology positioning`;
  } else if (!hasUserProductGuidance && isWatchKeywordCombined) {
    return productGuidance + `- CRITICAL: This page is about WATCHES/TIMEPIECES (keyword or title: "${keyword}")
- You MUST write about watches (Grand Watch, Metawatch) ONLY. DO NOT mention ANY phone products or phone-only services
- DO NOT mention phones (Agent Q, Quantum Flip, Metavertu, iVERTU, Signature, etc.) - they are NOT relevant
...
- DO NOT include unrelated product information (e.g. do not write about phones when the topic is watches/腕表)
- DO NOT invent specifications, prices, materials, or features for VERTU products
- DO NOT use external knowledge for product details or brand-specific claims; general topic context may be used only when factual and non-product
...
- Use the knowledge base data extensively to create rich, detailed descriptions. Aim for substantial, informative content that gives the reader real value.
- Quote specific numbers, measurements, and technical details from the knowledge base. Where helpful, present them in a clear table (e.g. "Key specifications" or "Features at a glance").
- Include 1–2 relevant images (using the allowed image URLs and <figure>/<figcaption>) and at least one HTML table so the page has both visual and structural richness.
- Reference actual features, materials, and services mentioned in the knowledge base. Create engaging narratives around the verified facts.
...
function filterKnowledgeBaseByProductType(
  ...
  const isPhoneKeyword = /\b(phone|phones|smartphone|smart phone|cellphone|cell phone|mobile)\b/i.test(combined);
  const isWatchKeyword = /(watch|watches|timepiece|timepieces|wristwatch|horology|chronograph|腕表|手表)/i.test(combined);
  const isRingKeyword = /\b(ring|rings|jewellery|jewelry|jewel|首饰)\b/i.test(combined);
...
function isArticleRich(
  ...
  const hasQuestionHeadings = questionHeadings.length >= 1;
