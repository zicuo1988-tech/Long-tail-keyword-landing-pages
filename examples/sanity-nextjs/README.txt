...
1) 环境变量（Next 前台，只读，用 public dataset + CDN）
   ...
   NEXT_PUBLIC_SITE_URL=https://vertu.com
   （用于 generateMetadata 的 canonical / Open Graph；与 SANITY_BASE_URL 站点根一致，勿带尾部斜杠）
...
7) 发布到 Sanity 时后端会写入 bodyHtml（仅 body+内联 style）、jsonLd（JSON 数组字符串）、canonicalPath、ogImage、primaryCategory。Next 示例在页面顶层输出 JSON-LD，正文只渲染 bodyHtml，避免嵌套完整 HTML 文档。

7b) 表格样式：正文页 wrapper 须带 class="luxury-life-guide-body"（见 page.tsx）。layout.tsx 已全局 import frontend/shared/landing-base.css，表格边框/表头样式才会生效。旧文档若发布于修复前，需重新生成并发布一次（bodyHtml 会带上 .ll-landing-root + 改写后的 CSS）。

8) 合并到已有 Next 项目：复制 schema 到 Studio；复制 app/luxury-life-guides 与 lib/sanity.client.ts；安装依赖见 package.json；配置 NEXT_PUBLIC_SANITY_*。务必在根 layout 中 import 本仓库 frontend/shared/landing-base.css（或把该文件拷入对方项目后 import），否则仅靠 bodyHtml 内联 style 时表格边框可能仍被主题 CSS 覆盖。
